...
E03 更新为命令式 CLI 与默认非阻塞 TUI 并存。单个数据目录登记一个稳定实例和唯一 MASTER 权威会话；前端均为视图，持久排队与冻结前缀由 Core/Store 管理。旧多会话经过离线显式选定，未选历史只读；任务局部上下文与内部会话不成为第二个可写主会话。追加 002，不重写 001 或旧请求。

这是实施过程中发现的权威归属及交互阻塞缺陷，经 Ayanami DeepSeek 同意。终端依赖 BubbleTea/Bubbles/x/ansi 已锁定；完整实施正文见 SingleConversationTUI、EngineeringArchitecture、Interfaces、Storage。本轮验收只执行 AUTH/TUI/DOC/BUILD，原 M1–M3 的全仓/长时验收不重跑。
