...
func Reply(w http.ResponseWriter, code int, id string, result any, err error) {
	...
	if err != nil {
		...
		case "NOT_FOUND", "QUESTION_NOT_FOUND_IN_SESSION":
			code = 404
		case "AUTHORITY_BUSY", "AUTHORITY_TURN_NOT_HEAD", "IDEMPOTENCY_CONFLICT", "CONFLICT", "STALE_FENCE", "QUESTION_ALREADY_RESOLVED", "QUESTION_CAPACITY_EXCEEDED":
			code = 409
		case "UNAUTHENTICATED":
			code = 401
		case "AUTHORITY_SESSION_MISMATCH", "PERMISSION_DENIED", "OUTPUT_CLASS_UNKNOWN", "DISCLOSURE_DENIED", "QUESTION_AUTHORITY_DENIED":
			code = 403
