...
func Reply(w http.ResponseWriter, code int, id string, result any, err error) {
	...
	if err != nil {
		...
		case "NOT_FOUND":
			code = 404
		case "IDEMPOTENCY_CONFLICT", "CONFLICT", "STALE_FENCE":
			code = 409
		case "UNAUTHENTICATED":
			code = 401
		case "PERMISSION_DENIED", "DISCLOSURE_DENIED":
			code = 403
...
func PublicCode(err error) string {
	...
	for _, c := range []string{"IDEMPOTENCY_CONFLICT", "CONTEXT_REQUIRED_OVERFLOW", "DISCLOSURE_DENIED", "PERMISSION_DENIED", "UNAUTHENTICATED", "INPUT_TOO_LARGE", "BACKPRESSURE", "BUDGET_EXHAUSTED", "STALE_FENCE", "CONFLICT", "DEPENDENCY_UNAVAILABLE", "EXECUTION_FROZEN", "UNSUPPORTED_VERSION", "UNKNOWN_CAPABILITY"} {
