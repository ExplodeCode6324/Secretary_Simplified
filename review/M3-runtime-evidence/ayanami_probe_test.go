...
func TestAyanamiProbeArtifactCancelMustDeny(t *testing.T) {
	...
	if e != nil { t.Fatal(e) }
	if e = s.CancelTask(context.Background(), run.TaskID); e != nil { t.Fatal(e) }
...
func TestAyanamiProbeExpiredPermitMustDenyLocalEffect(t *testing.T) {
	...
	if e != nil { t.Fatal(e) }
	if _, e = s.DispatchRun(context.Background(), claimed, "worker", now); e != nil { t.Fatal(e) }
	var permitID string
	if e = s.DB.QueryRow(`SELECT id FROM execution_permit WHERE run_id=?`, run.ID).Scan(&permitID); e != nil { t.Fatal(e) }
	past := contract.Timestamp(time.Now().Add(-time.Minute))
	var payload string
	if e = s.DB.QueryRow(`SELECT payload_json FROM execution_permit WHERE id=?`, permitID).Scan(&payload); e != nil { t.Fatal(e) }
	var m map[string]any
	if e = json.Unmarshal([]byte(payload), &m); e != nil { t.Fatal(e) }
	m["expires_at"] = past
	b, _ := json.Marshal(m)
	if _, e = s.DB.Exec(`UPDATE execution_permit SET expires_at=?,payload_json=? WHERE id=?`, past, string(b), permitID); e != nil { t.Fatal(e) }
...
func TestAyanamiProbeCancelledUnknownReconcileMustPreserveCancellation(t *testing.T) {
	...
	if e != nil { t.Fatal(e) }
	if _, e = s.DispatchRun(ctx, claimed, "worker", now); e != nil { t.Fatal(e) }
	if e = s.RecordNotification(ctx, claimed); e != nil { t.Fatal(e) }
	if _, e = s.ClaimRun(ctx, "recovery", now.Add(31*time.Second)); e != sql.ErrNoRows { t.Fatal(e) }
	if e = s.CancelTask(ctx, run.TaskID); e != nil { t.Fatal(e) }
	if ok, e := s.ReconcileLocal(ctx, run.ID, dir); e != nil || !ok { t.Fatalf("reconcile: %v %v", ok, e) }
	var state string
	if e = s.DB.QueryRow(`SELECT state FROM task WHERE id=?`, run.TaskID).Scan(&state); e != nil { t.Fatal(e) }
	if state == "SUCCEEDED" { t.Fatal("reconciliation rewrote cancelled task to SUCCEEDED") }
}
...
func TestAyanamiProbeMutedAlarmPersistenceAndExpiry(t *testing.T) {
	...
	if e != nil { t.Fatal(e) }
	if _, e = s.DispatchRun(context.Background(), claimed, "muted-runner", now); e != nil { t.Fatal(e) }
	if e = s.MutedAlarm(context.Background(), claimed); e != nil { t.Fatal(e) }
	var state string
	if e = s.DB.QueryRow(`SELECT state FROM alarm_session WHERE run_id=?`, run.ID).Scan(&state); e != nil { t.Fatal(e) }
	if state != "PLAYING" { t.Fatalf("alarm state %s", state) }
	if e = s.ExpireMutedAlarms(context.Background(), time.Now().Add(10*time.Second)); e != nil { t.Fatal(e) }
	if e = s.DB.QueryRow(`SELECT state FROM alarm_session WHERE run_id=?`, run.ID).Scan(&state); e != nil { t.Fatal(e) }
	if state != "STOPPED" { t.Fatalf("expired alarm state %s", state) }
}
...
func TestAyanamiProbeRejectedDispatchMustRefundActiveBudget(t *testing.T) {
	...
	if e = s.PutGrant(context.Background(), g); e != nil { t.Fatal(e) }
	if _, e = s.DispatchRun(context.Background(), claimed, "worker", time.Now().Add(time.Second)); e == nil { t.Fatal("revoked grant dispatched") }
	var root, raw string
	if e = s.DB.QueryRow(`SELECT root_id FROM task WHERE id=?`, run.TaskID).Scan(&root); e != nil { t.Fatal(e) }
	if e = s.DB.QueryRow(`SELECT payload_json FROM root_budget WHERE root_id=?`, root).Scan(&raw); e != nil { t.Fatal(e) }
	var budget map[string]any
	if e = json.Unmarshal([]byte(raw), &budget); e != nil { t.Fatal(e) }
	if used, _ := budget["active_ms_used"].(float64); used != 0 { t.Fatalf("active budget leaked: %v", used) }
}
